document.addEventListener('DOMContentLoaded', function (event) {
  ...
  function flipCard(e) {
    ...
    if (target.classList.contains('card')) {
      if (target.classList.contains('selected')) {
        ...
        if (timerRunning == 0) {
          // checking if timer function is running
          timerRunning++
  ...
  // FUNCTION FOR POPS UP WITH CONGRATULATIONS
  function popupModal() {
    bodyId.style.display = 'none'
    modalId.style.display = 'inline'
    movesId.innerHTML =
      'You complete it in <span class="strong">' + moves + '</span> moves.'
    // THE STAR SYSTEM - CHANGING FROM SOLID TO EMPTY
    if (moves < 16) {
      scoreId.innerHTML = 'Your got ' + fullStar + fullStar + fullStar
    } else if (moves > 15 && moves < 21) {
      scoreId.innerHTML = 'Your got ' + fullStar + fullStar + emptyStar
    } else {
      scoreId.innerHTML = 'Your got ' + fullStar + emptyStar + emptyStar
    }
  }
  ...
  function timer() {
    ...
    if (counter == iconsArray.length) {
      stopTimer()
      timeId.innerHTML =
        "With in a time of  <span class='strong'> " +
        mins +
        ':' +
        secs +
        ' </span>'
